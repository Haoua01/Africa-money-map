document.addEventListener("DOMContentLoaded", async function () {
    ...
    const geoJsonFile = "data/Indicators/Hybrid/scores_hybrid.geojson"; 
            ...
            function populateCommuneDropdown(filteredCountryData, country, region, department) {
                ...
                communes.forEach(comm => {
                    ...
                    communeItem.addEventListener("click", () => {
                        ...
                        // updatePercentage(filteredCommuneData, country, comm);
                        updateMapDisplay();
    ...
    */
            ...
            onEachFeature: function (feature, layer) {
                ...
                layer.on({
                    ...
                    click: function (e) {
                        ...
                        // Call updateStats with the filtered data
                        updatePercentage(filteredCommuneData, country, commune);
                        document.getElementById("countryDropdown").textContent = country;
                        document.getElementById("regionDropdown").textContent = region;
                        document.getElementById("departmentDropdown").textContent = department;
                        document.getElementById("communeDropdown").textContent = commune;
    ...
    function updatePercentage(filteredData, country, commune) {
        const popCountries = ['Nigeria', 'Cameroon', 'Ghana', 'Chad', 'Benin', 'Burkina Faso', 'Ivory Coast', 'Guinea-Bissau', 'Mali', 'Niger', 'Senegal', 'Togo'];
        if (popCountries.includes(country)) {
            // retrieve value of "percent" column for ADM3_FR="commune" and group_inde=15
            const isochrone15 = filteredData.find(feature => feature.properties.ADM3_FR === commune && feature.properties.group_inde === 15)?.properties.percent || 0;
            const isochrone30 = filteredData.find(feature => feature.properties.ADM3_FR === commune && feature.properties.group_inde === 30)?.properties.percent || 0;
            const isochrone45 = filteredData.find(feature => feature.properties.ADM3_FR === commune && feature.properties.group_inde === 45)?.properties.percent || 0;
            const isochrone60 = filteredData.find(feature => feature.properties.ADM3_FR === commune && feature.properties.group_inde === 60)?.properties.percent || 0;

            //const TotalIsochrone15 = filteredData.reduce((sum, feature) => sum + (Number(feature.properties.ADM3_FR === commune && feature.properties.group_inde === 15)?.properties.area_iso || 0), 0);
            //const TotalIsochrone30 = filteredData.reduce((sum, feature) => sum + (Number(feature.properties.ADM3_FR === commune && feature.properties.group_inde === 30)?.properties.area_iso || 0), 0);
            //const TotalIsochrone45 = filteredData.reduce((sum, feature) => sum + (Number(feature.properties.ADM3_FR === commune && feature.properties.group_inde === 45)?.properties.area_iso || 0), 0);
            //const TotalIsochrone60 = filteredData.reduce((sum, feature) => sum + (Number(feature.properties.ADM3_FR === commune && feature.properties.group_inde === 60)?.properties.area_iso || 0), 0);

            document.getElementById("num-municipalities").innerHTML = `<span>${isochrone15}%of coverage within 0-15min</span>`;
            document.getElementById("total-bran").innerHTML = `<span>${isochrone30}%of coverage within 15-30min</span>`;
            document.getElementById("percent-pop").innerHTML = `<span>${isochrone45}%of coverage within 30-45min</span>`;
            document.getElementById("percent-area").innerHTML = `<span>${isochrone60}%of coverage within 45-60min</span>`;
        } else {
            // If the country is not in the UEMOA group, don't update stats
            console.log(`No stats update for ${country}.`);
            // Clear the contents of the statistics divs
